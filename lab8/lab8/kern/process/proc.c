...
do_fork(uint32_t clone_flags, uintptr_t stack, struct trapframe *tf) {
    ...
    */
    ...
    if((ret=copy_files(clone_flags,proc)!=0)){
        cprintf("copy thread failed:bad_copy_file\n");
...
load_icode(int fd, int argc, char **kargv) {
    ...
    //(3.1) read raw data content in file and resolve elfhdr
    struct elfhdr elfs;
    struct elfhdr *elf=&elfs;
    load_icode_read(fd,elf,sizeof(struct elfhdr),0);
    
    //这里面，谁说只有一个ph的呢，好像是有比较很多个，至少就下面需要用到的 for循环而言，所以要乘上elf->e_phnum
    ...
    for (int i =0; i < elf->e_phnum; i ++) {
        ...
        load_icode_read(fd,ph,sizeof(struct proghdr),elf->e_phoff+sizeof(struct proghdr)*i);
