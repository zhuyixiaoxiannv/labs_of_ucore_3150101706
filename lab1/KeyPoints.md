...
在lab4当中提到了TSS段，我之前对这个内容也不是特别清楚，在此补充。

中断触发后除了保存寄存器现场，还有一个切换栈的操作，这是避免不同的程序相互影响彼此的栈空间。（这里面对应的是TSS在中断切换的作用，其实就是从用户堆栈切换到内核堆栈，但是在内核中断的时候，TSS是不用切换的，因为就一个堆栈区——然而这边用了两个线程，那么需要考虑线程切换的时候的堆栈变化。

TSS对应了GDT表中的一个描述符。

![IA-32TSSandPT](.\IA-32TSSandPT.jpg)
